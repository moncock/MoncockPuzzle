...
async function mintSnapshot() {
  try {
    ...
    // 4) Warm gateways in background (non-blocking)
    (async () => { try { await warm(metaUri, 2); } catch {} })();
    (async () => { try { if (imgGateway) await warm(imgGateway, 1); } catch {} })();

    // 5) Mint on-chain (faster UX: wait 1 conf)
    setMintStatus('⛓️ Sending transaction…');
    const to = await signer.getAddress();
    const tx = await contract.mintNFT(to, metaUri);
